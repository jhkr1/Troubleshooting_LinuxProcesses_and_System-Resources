...
운영 환경에서 OOM이 발생하면 먼저 “누가 프로세스를 종료했는가”를 구분해야 한다. 커널 OOM Killer가 종료했는지, 애플리케이션 내부 보호 로직이 종료했는지, 컨테이너 런타임의 메모리 제한에 걸렸는지에 따라 조치가 달라진다.

OOM 대응에서 가장 흔한 실수는 프로세스를 바로 재시작하는 것이다. 재시작은 서비스를 일시적으로 살릴 수 있지만, 동시에 Heap 상태, 프로세스 상태, 마지막 로그 같은 중요한 증거를 지운다. 따라서 사용자가 이미 큰 영향을 받고 있는 상황이 아니라면, 먼저 짧게라도 증거를 확보하는 것이 좋다.

즉시 대응은 다음 순서로 진행한다. 여기서 목표는 “서비스 영향 줄이기”와 “원인 분석에 필요한 증거 보존”을 동시에 달성하는 것이다.

```text
1. 로그와 메트릭을 먼저 보존한다.
2. RSS, Heap, 컨테이너 memory usage, OOM Killer 로그를 확인한다.
3. 최근 배포, 트래픽 증가, 배치 작업, 캐시 증가 여부를 확인한다.
4. 필요한 경우 임시로 메모리 limit 또는 replica 수를 늘린다.
5. 재시작은 증거를 확보한 뒤 수행한다.
```
...
| 단계 | 왜 필요한가 |
|---|---|
| 로그와 메트릭 보존 | 재시작 후에는 종료 직전 상태를 복원하기 어렵다 |
| RSS/Heap 확인 | 실제 메모리가 늘었는지, 앱 내부 Heap만 늘었는지 구분한다 |
| OOM Killer 로그 확인 | 커널이 죽였는지 앱이 스스로 죽었는지 판단한다 |
| 최근 변경 확인 | 메모리 문제는 배포, 트래픽, 데이터 크기 변화 직후 자주 드러난다 |
| limit/replica 조정 | 원인 제거 전까지 사용자 영향을 줄이는 임시 완화책이다 |

메모리 제한을 높이는 것은 빠른 완화책이 될 수 있다. 예를 들어 정상 peak가 400MB인데 limit이 512MB라면, 일시적인 트래픽 증가만으로도 위험해질 수 있다. 이 경우 limit을 넉넉히 조정하는 것은 합리적이다. 그러나 누수가 있는 프로그램은 더 큰 메모리를 받아도 시간이 지나면 다시 같은 한계에 도달한다. 그래서 limit 상향은 “시간을 버는 조치”이지 “원인을 제거하는 조치”가 아니다.
...
예를 들어 캐시가 원인이라면 캐시를 전부 없애는 것이 정답은 아닐 수 있다. 캐시는 성능을 위해 필요하다. 대신 최대 크기, TTL, eviction 정책을 두어 “얼마나 커질 수 있는지”를 제한해야 한다. 큰 파일이나 큰 요청이 원인이라면 전체 데이터를 한 번에 메모리에 올리지 말고 streaming 방식으로 나누어 처리하는 편이 안전하다.
...
알림은 너무 늦게 울리면 의미가 없다. 메모리 사용률이 99%가 되었을 때 처음 알림이 오면 대응할 시간이 거의 없다. 보통은 경고 단계와 위험 단계를 나누어 둔다.
...
이렇게 단계적으로 알림을 설계하면 장애가 사용자에게 보이기 전에 원인을 확인하고, 필요하면 트래픽 우회나 replica 증설 같은 조치를 먼저 수행할 수 있다.
...
CPU Spike가 발생하면 먼저 CPU를 많이 쓰는 주체를 확인한다. 전체 노드 CPU가 높은지, 특정 프로세스만 높은지, 특정 스레드가 높은지, 또는 애플리케이션 내부 지표만 높은지 구분해야 한다.

CPU 문제는 겉으로는 모두 비슷해 보인다. 그래프에는 CPU 사용률이 높게 나오고, 사용자는 서비스가 느리다고 느낀다. 그러나 원인은 여러 가지일 수 있다. 트래픽이 갑자기 늘어난 것일 수도 있고, 특정 API의 알고리즘이 비효율적일 수도 있으며, 무한 루프나 busy wait처럼 코드 결함이 숨어 있을 수도 있다.

즉시 대응은 다음 순서로 진행한다. 목표는 CPU를 쓰는 위치를 좁히고, 사용자 요청이 계속 밀리지 않도록 압력을 낮추는 것이다.

```text
1. top, ps, pidstat, profiler로 CPU를 쓰는 PID와 스레드를 확인한다.
2. 최근 배포, 트래픽 증가, 특정 API 호출 증가를 확인한다.
3. CPU-bound 작업이 요청 처리 경로에 있는지 확인한다.
4. 임시로 replica를 늘리거나 rate limit을 적용한다.
5. 필요하면 무거운 작업을 비동기 큐나 별도 워커로 우회한다.
```
...
| 단계 | 왜 필요한가 |
|---|---|
| PID와 스레드 확인 | 노드 전체 문제인지 특정 앱 문제인지 구분한다 |
| 최근 변경 확인 | CPU Spike는 배포 직후 새 코드 경로에서 자주 발생한다 |
| API 호출 증가 확인 | 특정 endpoint가 CPU를 대부분 사용할 수 있다 |
| rate limit 적용 | 서버가 처리할 수 있는 양 이상으로 요청이 쌓이는 것을 막는다 |
| 작업 큐 분리 | 오래 걸리는 계산을 사용자 요청 흐름에서 떼어낸다 |

CPU 문제는 단순히 “CPU가 높다”가 아니라 “CPU를 쓰는 동안 다른 작업이 기다린다”는 점이 중요하다. 사용자 요청을 처리하는 스레드가 긴 계산을 오래 붙잡고 있으면 응답 시간이 늘어난다. 이벤트 루프 기반 서버라면 하나의 CPU-bound 작업이 전체 요청 처리를 막을 수도 있다.

예를 들어 이미지 변환, 대용량 JSON 파싱, 복잡한 암호화 연산, 큰 반복문은 모두 CPU-bound 작업이 될 수 있다. 이런 작업이 HTTP 요청 안에서 바로 실행되면 그 요청뿐 아니라 같은 프로세스의 다른 요청까지 늦어질 수 있다. 이때는 작업을 큐에 넣고 별도 워커가 처리하게 만들거나, 작업을 작은 단위로 쪼개서 실행 시간을 제한하는 편이 안전하다.

실무에서 자주 쓰는 완화책은 다음과 같다.

| 상황 | 빠른 조치 | 근본 개선 |
|---|---|---|
| 트래픽 급증 | autoscaling, replica 증설 | capacity planning, 부하 테스트 |
| 특정 API 과부하 | rate limit, timeout | 알고리즘 개선, 쿼리 최적화 |
| 긴 계산 작업 | 작업 큐로 분리 | chunking, 비동기 처리, 캐싱 |
| 무한 루프 또는 busy wait | 프로세스 격리, 롤백 | 종료 조건, sleep/backoff 추가 |
| GC 또는 런타임 부하 | heap/GC 지표 확인 | 객체 생성량 감소, 런타임 튜닝 |

빠른 조치와 근본 개선은 구분해야 한다. replica를 늘리면 같은 트래픽을 여러 프로세스가 나누어 처리하므로 당장은 CPU 압력이 낮아질 수 있다. 하지만 특정 코드가 요청 한 번마다 불필요하게 큰 계산을 한다면, replica를 늘려도 비용은 계속 증가한다. 결국 알고리즘, 쿼리, 캐시, 비동기 처리 구조를 개선해야 한다.

예방을 위해서는 CPU 사용률과 응답 시간을 함께 본다. CPU가 높아도 응답 시간이 안정적이면 처리량이 많은 정상 상태일 수 있다. 반대로 CPU가 아주 높지 않아도 p95, p99 latency가 튀면 사용자 경험에는 장애로 보일 수 있다.
...
Deadlock은 프로세스가 죽지 않을 수 있기 때문에 탐지가 늦다. CPU가 낮고 에러 로그도 많지 않은데 요청이 끝나지 않는 형태로 나타날 수 있다. 따라서 “살아 있음”과 “정상 동작 중”을 구분해야 한다.

Deadlock 대응에서 중요한 점은 재시작 전에 실행 흐름 증거를 확보하는 것이다. Deadlock은 재시작하면 사라진다. 겉으로는 문제가 해결된 것처럼 보이지만, 어떤 스레드가 어떤 락을 들고 있었는지에 대한 증거도 함께 사라진다. 그래서 가능한 경우 thread dump나 stack trace를 먼저 남긴다.

즉시 대응은 다음 순서로 진행한다. 목표는 멈춘 인스턴스를 서비스 경로에서 빼고, 동시에 원인 분석에 필요한 스레드 상태를 확보하는 것이다.

```text
1. 프로세스가 살아 있는지 확인한다.
2. 스레드 dump, stack trace, goroutine dump 같은 실행 흐름 증거를 확보한다.
3. 같은 락이나 자원을 기다리는 스레드가 있는지 확인한다.
4. 영향 범위를 줄이기 위해 해당 인스턴스를 로드밸런서에서 제외한다.
5. 증거 확보 후 재시작 또는 롤백으로 서비스를 복구한다.
```
...
| 단계 | 왜 필요한가 |
|---|---|
| 프로세스 생존 확인 | 크래시와 무응답을 구분한다 |
| thread dump 확보 | 어떤 스레드가 어디서 멈췄는지 확인한다 |
| lock wait 확인 | 순환 대기 구조가 있는지 판단한다 |
| 로드밸런서 제외 | 멈춘 인스턴스로 새 요청이 들어가지 않게 한다 |
| 재시작 또는 롤백 | 사용자 영향을 줄이기 위한 복구 조치다 |

Deadlock은 재시작하면 당장은 풀릴 수 있다. 그러나 재시작은 상태를 초기화할 뿐 락 순서 문제를 고치지 않는다. 같은 실행 경로가 다시 열리면 같은 Deadlock이 재발할 수 있다.

근본 해결은 순환 대기가 생기지 않도록 설계를 바꾸는 것이다.

| 예방 방법 | 설명 |
|---|---|
| 락 획득 순서 통일 | 모든 코드가 항상 같은 순서로 락을 잡는다 |
| try-lock과 timeout | 두 번째 락을 못 잡으면 첫 번째 락을 풀고 재시도한다 |
| critical section 축소 | 락을 잡고 있는 시간을 짧게 만든다 |
| 락 중첩 제거 | 가능하면 한 번에 하나의 락만 잡도록 구조를 단순화한다 |
| 공유 상태 축소 | 큐, actor, 단일 writer 구조로 공유 자원 접근을 제한한다 |
| 코드 리뷰 규칙 | 새 락 추가 시 락 순서와 timeout 여부를 검토한다 |

가장 기본적인 예방책은 락 획득 순서를 통일하는 것이다. 예를 들어 어떤 코드에서는 `UserLock -> OrderLock` 순서로 잡고, 다른 코드에서는 `OrderLock -> UserLock` 순서로 잡으면 순환 대기가 생길 수 있다. 모든 코드가 항상 `UserLock -> OrderLock` 순서를 지키면 이 가능성을 크게 줄일 수 있다.

`try-lock`과 timeout도 중요한 방어 장치다. 두 번째 락을 일정 시간 안에 잡지 못했다면 첫 번째 락을 계속 들고 기다리지 말고 내려놓아야 한다. 그래야 다른 스레드가 진행할 기회를 얻는다. 이것은 “기다리되 영원히 기다리지는 않는다”는 원칙이다.

critical section을 줄이는 것도 효과적이다. 락을 잡은 상태에서는 가능한 한 짧은 코드만 실행해야 한다. 네트워크 호출, 파일 I/O, 긴 계산, 외부 API 호출을 락 안에서 수행하면 다른 스레드가 오래 기다릴 수 있다. 락은 공유 데이터를 읽거나 수정하는 아주 짧은 구간에만 사용하는 것이 좋다.
...
이런 신호가 함께 보이면 스레드 dump를 우선 확보해야 한다. Deadlock 분석에서 가장 강한 증거는 “누가 어떤 락을 들고 있고, 누가 그 락을 기다리는가”를 보여 주는 실행 흐름이다.

예방을 위해서는 테스트도 필요하다. 단위 테스트만으로는 Deadlock을 찾기 어렵다. 여러 스레드가 동시에 같은 코드를 실행하는 동시성 테스트, timeout을 걸어 두는 통합 테스트, 부하 상황에서 요청이 끝까지 완료되는지 확인하는 테스트가 도움이 된다. 특히 락을 새로 추가하거나 공유 자원 접근 방식을 바꾸는 변경은 코드 리뷰에서 더 신중하게 봐야 한다.
